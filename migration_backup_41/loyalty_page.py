...
def loyalty_kpis(db, rows):
    ...
    logs = _rows(
        ...
        (month,))
    top_customer = rows[0].get("name") if rows else "—"
    return {
        "customers": len(rows),
...
class LoyaltyPointsDialog(QDialog):
    def __init__(self, db, user="Admin", customer=None, parent=None):
        ...
        for source in self.customers:
            self.customer_box.addItem(
                f"{source['name']} · {source['phone'] or source['code']} "
    ...
    def _update_hint(self):
        ...
        self.balance_hint.setText(
            f"{points:,} points available · Tier: {tier_for_points(points)}")
...
class LoyaltySettingsDialog(QDialog):
    def __init__(self, db, parent=None):
        ...
        hint = QLabel(
            "Tier rules: Bronze 0+ · Silver 500+ · Gold 1500+ · Platinum 3000+")
...
class LoyaltyPage(QWidget):
    def __init__(self, db_name, current_user="Admin", parent=None):
        ...
        self.rules_btn = _button("⚙ Loyalty Rules")
        ...
        self.kpi_labels = {}
        specs = [
            ("customers", "Loyalty Customers", "👥", C["blue"]),
            ("active_points", "Active Point Balance", "⭐", C["warning"]),
            ("earned_month", "Earned This Month", "➕", C["success"]),
            ("redeemed_month", "Redeemed This Month", "🎁", C["accent"]),
            ("gold_plus", "Gold / Platinum", "👑", "#5856D6"),
            ("top_customer", "Top Loyalty Customer", "🏆", "#00A67E"),
        ]
        grid = QGridLayout()
        grid.setSpacing(9)
        ...
        self.search.setPlaceholderText("Customer name, phone or code…")
        ...
        actions = QGridLayout()
        self.add_selected_btn = _button("⭐ Add / Redeem")
        self.edit_customer_btn = _button("✏ Edit Customer")
    ...
    def _show_drawer(self, customer):
        ...
        self.drawer_summary.setText(
            ...
            f"{f' · {needed:,} points needed' if next_tier else ''}")
        history = _rows(
