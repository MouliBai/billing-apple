...
from customer_page import init_customer_tables, get_customer_invoices
...
class PaymentDialog(QDialog):
    def __init__(self, db, customer, user="Admin", parent=None):
        ...
        self.amount.setPrefix("₹ "); self.amount.setValue(
    ...
    def _save(self):
        ...
        with sqlite3.connect(self.db) as conn:
            ...
            """, (
                ...
                f"{receipt} · {self.notes.toPlainText().strip()}",
                self.user, now))
        self.accept()
...
class CreditManagementPage(QWidget):
    def __init__(self, db_name, current_user="Admin", parent=None):
        ...
        self.kpi_labels = {}
        specs = [
            ("due", "Total Credit Due", "💳", C["blue"]),
            ("overdue", "Overdue Amount", "⛔", C["accent"]),
            ("customers", "Credit Customers", "👥", "#5856D6"),
            ("overdue_customers", "Overdue Customers", "⚠️", C["warning"]),
            ("today", "Due Today", "📅", "#FF9500"),
            ("collection", "Collection This Month", "💰", C["success"]),
        ]
        ...
        self.search.setPlaceholderText("Customer name, phone or bill number…")
        ...
        actions = QGridLayout()
        self.payment_btn = _button("💳 Add Payment", True)
        self.reminder_btn = _button("🔔 Log Reminder")
        self.call_btn = _button("📞 Call Customer")
        self.block_btn = _button("⛔ Block Credit")
    ...
    def _show_drawer(self, customer):
        ...
        self.drawer_summary.setText(
            ...
            f"<b>Oldest Due:</b> {customer.get('oldest_due_date') or '—'}<br>"
