...
from product_page import C, FIELD_SS, _NO_ARROW, _apply_combo_delegate
...
class CustomerFormDialog(QDialog):
    def __init__(self, db, current_user="Admin", customer=None, parent=None):
        ...
        for widget in (self.credit_limit, self.credit_balance):
            widget.setRange(0, 999999999); widget.setPrefix("₹ ")
...
class CustomerPage(QWidget):
    def __init__(self, db_name, current_user="Admin", navigate_cb=None, parent=None):
        ...
        self.kpi_labels = {}
        specs = [
            ("total", "Total Customers", "👥", C["blue"]),
            ("active", "Active Customers", "✅", C["success"]),
            ("credit", "Credit Customers", "💳", C["warning"]),
            ("due", "Total Credit Due", "⏳", C["accent"]),
            ("repeat", "Repeat Customers", "🔁", "#5856D6"),
            ("month", "This Month Customers", "📅", "#00A67E"),
        ]
        ...
        self.search.setPlaceholderText("Name, phone, customer code or GSTIN…")
        ...
        self.detail_kpis = QLabel("—"); self.detail_kpis.setWordWrap(True)
        ...
        actions = QGridLayout()
        self.edit_btn = _button("✏ Edit Customer")
        self.sale_btn = _button("🧾 New Sale", True)
        self.payment_btn = _button("💳 Add Payment")
        self.block_btn = _button("⛔ Disable / Block")
    ...
    def _fill(self, table, rows, columns, money_columns=()):
        ...
        for source in rows:
            ...
            for col, key in enumerate(columns):
                ...
                text = _money(value) if col in money_columns else str(value or "—")
    ...
    def _show_drawer(self, customer):
        ...
        self.drawer_summary.setText(
            ...
            f"<b>Type:</b> {customer.get('customer_type') or '—'}<br>"
            f"<b>City:</b> {customer.get('city') or '—'}<br>"
            f"<b>Status:</b> {customer.get('status') or '—'}<br>"
            f"<b>Credit Balance:</b> {_money(customer.get('credit_balance'))}<br>"
            f"<b>Loyalty:</b> {customer.get('loyalty_points',0)} points · "
            f"{customer.get('loyalty_tier') or 'Bronze'}")
        self.detail_kpis.setText(
            f"Bills: {customer.get('total_bills',0)}   ·   "
            f"Purchases: {_money(customer.get('total_purchase_value'))}   ·   "
            f"Average Bill: {_money(customer.get('average_bill_value'))}\n"
            f"Returns: {customer.get('return_count',0)}   ·   "
            f"Last Purchase: {customer.get('last_purchase_date') or '—'}")
    ...
    def _new_sale(self):
        ...
        QMessageBox.information(
            ...
            f"{self._selected.get('name')} · {self._selected.get('phone')}\n\n"
    ...
    def _add_payment(self):
        ...
        amount = QDoubleSpinBox(); amount.setRange(0, 999999999); amount.setPrefix("₹ ")
