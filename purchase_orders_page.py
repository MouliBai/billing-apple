...
def purchase_kpis(db):
    ...
    return {
        ...
        "suppliers": row[3] or 0, "last": row[4] or "—", "stock": qty or 0,
    }
...
class PurchaseInvoiceDialog(QDialog):
    def __init__(self, db, current_user="Admin", parent=None, invoice=None):
        ...
        self.product_map = {
            f"{row['item_code']} · {row['name']}": row for row in self.products}
        ...
        self.supplier_code = QLabel("—"); self.gstin = QLabel("—")
        self.phone = QLabel("—"); self.terms = QLabel("—")
        self.balance = QLabel("—")
        ...
        self.paid.setPrefix("₹ "); self.paid.valueChanged.connect(self._recalc)
        self.gross_lbl = QLabel("₹0.00"); self.gst_lbl = QLabel("₹0.00")
        self.discount_lbl = QLabel("₹0.00"); self.net_lbl = QLabel("₹0.00")
        self.balance_lbl = QLabel("₹0.00"); self.status_lbl = QLabel("Pending")
    ...
    def _supplier_changed(self, *_):
        ...
        self.supplier_code.setText(supplier.get("code") or "—")
        self.gstin.setText(supplier.get("gstin") or "—")
        self.phone.setText(
            supplier.get("phone") or supplier.get("mobile_number") or "—")
    ...
    def _add_line(self):
        ...
        for col in range(1, 5):
            self.lines.setItem(row, col, QTableWidgetItem("—"))
        ...
        remove = QPushButton("✕"); remove.setFixedSize(28, 28)
    ...
    def _product_changed(self, row):
        ...
        for col, value in enumerate(values, 1):
            self.lines.setItem(row, col, QTableWidgetItem(str(value or "—")))
...
class PurchaseOrdersPage(QWidget):
    def __init__(self, db_name, current_user="Admin", parent=None):
        ...
        specs = [
            ("total", "Total Purchase Invoices", "🧾", C["blue"]),
            ("month", "This Month Purchase", "📅", "#5856D6"),
            ("pending", "Pending Payments", "⏳", C["accent"]),
            ("stock", "Total Stock Added", "📦", C["success"]),
            ("suppliers", "Active Suppliers", "🏭", C["warning"]),
            ("last", "Last Purchase", "🕒", "#AF52DE"),
        ]
        ...
        self.search.setPlaceholderText(
            "Invoice no, supplier, product code or product name…")
        ...
        self.drawer_summary = QLabel("—"); self.drawer_summary.setWordWrap(True)
    ...
    def _show_drawer(self, invoice):
        ...
        self.drawer_title.setText(f"Invoice · {number}")
        self.drawer_summary.setText(
            f"<b>Supplier:</b> {invoice.get('supplier_name') or '—'}<br>"
            f"<b>Invoice Date:</b> {invoice.get('invoice_date') or '—'}<br>"
            f"<b>Purchase Date:</b> {invoice.get('purchase_date') or '—'}<br>"
            ...
            f"<b>Status:</b> {invoice.get('payment_status') or '—'}<br>"
