"""
git_sync.py — Push the local SQLite DB to a private GitHub repo
══════════════════════════════════════════════════════════════════════
Fill in GITHUB_REPO_URL and GITHUB_TOKEN below (auth is hard-coded here
per your setup — nothing is read from env vars or prompts).

Behaviour:
  • Only the .db file is ever added / committed / pushed.
  • Commit message = current date & time, e.g. "2026-07-01 14:32:07".
  • Runs on a background QThread so the UI never freezes.
  • Called automatically right after a brand-new DB is created
...
# file ever lives here — nothing else from the app is copied in.
SYNC_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), ".db_sync")
    ...
    try:
        ...
        if r.returncode != 0:
            ...
            if "nothing to commit" in combined:
                return True, "DB already up to date on GitHub — nothing to push."
