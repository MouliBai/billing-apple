...
class MainWindow(QWidget):
    """
    Drop-in main window.  Add to your app entry point:

        from sidebar import MainWindow
        win = MainWindow(db_name="billing.db", company_name="Evo Aura")
        win.showMaximized()
    """
    ...
    def __init__(self, db_name=None, company_name=None, parent=None):
        super().__init__(parent)
        self.DB_NAME      = db_name      or self.DB_NAME
        self.COMPANY_NAME = company_name or self.COMPANY_NAME

        self.setWindowTitle(f"{self.COMPANY_NAME}  —  Evo Aura")
        ...
        self._sidebar.set_company(self.COMPANY_NAME)
        root.addWidget(self._sidebar)

        # ── Right area: topbar + page stack ──────────────────
        right = QWidget(); right.setStyleSheet("background:#F5F5F7;")
        rl = QVBoxLayout(right); rl.setContentsMargins(0,0,0,0); rl.setSpacing(0)

        # Topbar
        topbar = QWidget(); topbar.setFixedHeight(52)
        topbar.setStyleSheet("background:#FFFFFF;border-bottom:1px solid #D2D2D7;")
        tbl = QHBoxLayout(topbar); tbl.setContentsMargins(16,0,16,0); tbl.setSpacing(10)
        ...
        toggle_btn.setStyleSheet("""
            QPushButton {
                ...
                border-radius: 8px; font-size: 16px; color: #1D1D1F;
            }
            QPushButton:hover  { background: #E5E5EA; }
            QPushButton:pressed { background: #D2D2D7; }
        """)
        toggle_btn.clicked.connect(self._sidebar.toggle)

        self._page_title = QLabel("Dashboard")
        self._page_title.setFont(QFont("SF Pro Text", 14, QFont.Weight.Bold))
        self._page_title.setStyleSheet("color:#1D1D1F;background:transparent;")

        tbl.addWidget(toggle_btn)
        tbl.addSpacing(6)
    ...
    def _register_pages(self):
        """
        Register every page widget.  Only imports what is available —
        missing modules fall back to a placeholder label so the app
        still starts.
        """
        def _placeholder(key):
            w = QWidget(); w.setStyleSheet("background:#F5F5F7;")
            lbl = QLabel(f"📄  {key.replace('_',' ').title()}")
            lbl.setAlignment(Qt.AlignmentFlag.AlignCenter)
            lbl.setFont(QFont("SF Pro Text", 22, QFont.Weight.Bold))
            lbl.setStyleSheet("color:#A1A1A6;background:transparent;")
            lay = QVBoxLayout(w); lay.addWidget(lbl)
            return w

        # ── Products page ─────────────────────────────────────
        try:
            from product_page import ProductPage
            self._pages["product"] = ProductPage(
                self.DB_NAME,
                company_name = self.COMPANY_NAME,
            )
        except Exception as e:
            print(f"ProductPage load error: {e}")
            self._pages["product"] = _placeholder("product")

        # ── Suppliers page  ◀ WIRED HERE ──────────────────────
        try:
            from supplier_page import SupplierPage
            self._pages["suppliers"] = SupplierPage(
                db_name      = self.DB_NAME,
                company_name = self.COMPANY_NAME,
            )
        except Exception as e:
            print(f"SupplierPage load error: {e}")
            self._pages["suppliers"] = _placeholder("suppliers")

        # ── All other nav keys get placeholders ───────────────
        all_keys = [k for _, items in NAV_ITEMS for k, _, _ in items]
        for key in all_keys:
            if key not in self._pages:
                self._pages[key] = _placeholder(key)
    ...
    def _navigate(self, key: str):
        """Called by sidebar on every nav click."""
        if key not in self._pages:
            return
        self._stack.setCurrentWidget(self._pages[key])
        # Update topbar title
        label = next(
            (label for _, items in NAV_ITEMS for k, label, _ in items if k == key),
            key.replace("_", " ").title()
        )
