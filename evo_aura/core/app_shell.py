"""
EvoAura Desktop  â€”  PyQt6  |  SQLite  |  pyotp  |  qrcode
â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
EXACT BOOT FLOW:
  Start
    â””â”€ DB file exists?
        â”œâ”€ NO  â†’ AskDBName â†’ MasterAuth â†’ CompanySetup â†’ Signup â†’ QR â†’ Login
        â””â”€ YES â†’ Users exist?
                  â”œâ”€ NO  â†’ Signup â†’ QR â†’ Login
                  â””â”€ YES â†’ Login â†’ OTP â†’ Dashboard (embedded Sidebar)
...
# â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
#  DESIGN TOKENS  â€”  SINGLE SOURCE OF TRUTH FOR ALL COLOURS
#
...
# â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
C = dict(
    # â”€â”€ Backgrounds â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€ Accent â€” EvoAura brand red/pink â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€ Semantic colours â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€ Inputs & borders â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€ Gradient button variants (hex only, no #) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
class RightPanel(QWidget):
    ...
    def _build_brand(self):
        ...
        d = L("EvoAura â€” secure authentication, real-time analytics, "
    ...
    def show_features(self):
        self._clear()
        # â”€â”€ EvoAura description â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        self._dl.addWidget(L(
            ...
            "compliance effortlessly â€” built for speed, accuracy, and seamless "
            "day-to-day operations.",
            sz=11, col=C["text2"]))
        self._dl.addWidget(gap(8))
        self._dl.addWidget(L(
            "At Evo Aura, we empower businesses with intelligent billing solutions "
            "that go beyond basic invoicing â€” integrating cutting-edge data analytics, "
        ...
        # â”€â”€ Feature cards â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        for icon, title, desc in [
            ("ðŸ”", "Bank-grade Security", "TOTP 2FA with pyotp â€” real-time verified"),
            ("âš¡", "Lightning Fast",      "SQLite local storage â€” instant response"),
            ("ðŸ“Š", "Smart Dashboard",     "Real-time stats & business intelligence"),
            ("ðŸ¢", "Multi-tenant",        "Multiple company DBs from one install"),
            ("ðŸ§¾", "GST Compliance",      "Automated GST breakdowns & reports"),
            ("ðŸ“¦", "Inventory Control",   "Smart stock tracking & low-stock alerts"),
        ]:
            c = QWidget()
            c.setStyleSheet(
    ...
    def show_login_panel(self, usernames: list, db_name: str):
        ...
        if not usernames:
            self._dl.addWidget(
                L("No users yet â€” sign up first.", sz=11, col=C["text3"]))
        for u in usernames:
            row = QWidget()
            row.setStyleSheet(
                f"QWidget{{background:{C['bg_white']};"
    ...
    def show_signup_panel(self, db_name: str):
        ...
        if db_name:
            ...
            self._dl.addWidget(gap(4))
            ws = QLabel(f"ðŸ“‚  {db_name}")
            ws.setFont(F(11, mono=True))
            ws.setStyleSheet(f"color:{C['accent']};background:transparent;")
            self._dl.addWidget(ws)
            self._dl.addWidget(gap(16))
        for icon, tip in [
            ("ðŸ”’", "Use a strong password â€” 12+ characters"),
            ("ðŸ“±", "Install Google Authenticator or Authy first"),
            ("ðŸ’¾", "Store your 8 recovery codes somewhere safe"),
            ("ðŸ›¡ï¸", "Master code comes from your authenticator"),
        ]:
    ...
    def show_master_hint(self):
        ...
        self._dl.addWidget(gap(10))
        for icon, tip in [
            ("1ï¸âƒ£", "Open Google Authenticator or Authy"),
            ("2ï¸âƒ£", "Tap  +  â†’ Enter setup key manually"),
            ("3ï¸âƒ£", "Account name:  EvoAura Master"),
            ("4ï¸âƒ£", "Enter the secret key provided to you"),
            ("5ï¸âƒ£", "Type: TOTP  |  Time-based"),
            ("6ï¸âƒ£", "Enter the 6-digit code shown in the app"),
        ]:
            row = QHBoxLayout(); row.setSpacing(10)
            il = QLabel(icon); il.setFont(F(14))
        ...
        ccl.addWidget(L("ðŸ”‘  Need the Master Key?", sz=11, bold=True, col=C["blue"]))
        ccl.addWidget(L("Contact your EvoAura representative:", sz=10, col=C["text2"]))
        ccl.addWidget(L("ðŸ“§  evoaura.in@gmail.com", sz=10, col=C["blue"]))
        ccl.addWidget(L("ðŸ’¬  WhatsApp: +91 99944 84077", sz=10, col=C["blue"]))
    ...
    def show_otp_hint(self, username: str):
        ...
        self._dl.addWidget(gap(8))
        for icon, tip in [
            ("ðŸ“±", "Open your authenticator app"),
            ("ðŸ”", f"Find the entry for  {username}"),
            ("âŒ¨ï¸", "Enter the current 6-digit code"),
            ("â±ï¸", "Codes refresh every 30 seconds"),
            ("ðŸ†˜", "No phone? use a recovery code instead"),
        ]:
            row = QHBoxLayout(); row.setSpacing(10)
            il = QLabel(icon); il.setFont(F(14))
            il.setStyleSheet("background:transparent;"); il.setFixedWidth(24)
...
class V_AskDB(LV):
    ...
    def __init__(self):
        ...
        self._inp = SI("Company / Database Name", "ðŸ¢", "e.g. TechCorp, MyShop, Acme")
        ...
        btn = GB("Continue  â†’", full=True); btn.clicked.connect(self._submit)
...
class V_MasterAuth(LV):
    ...
    def __init__(self):
        ...
        icon = QLabel("ðŸ”"); icon.setFont(F(26))
        ...
        self._code = SI("Master TOTP Code", "ðŸ”‘", "6-digit code from authenticator", pw=True)
    ...
    def _verify(self):
        if verify_master(self._code.text()):
            self.sig_toast.emit("Master verified â€” database created.", "success")
            self._code.clear()
...
class V_CompanySettings(LV):
    ...
    def __init__(self):
        ...
        vl.addWidget(L("ðŸ¢  Company Settings", sz=21, bold=True))
        vl.addWidget(gap(6))
        ...
        self._logo_lbl = QLabel("ðŸ¢"); self._logo_lbl.setFont(F(22))
        ...
        self._cname = SI("Company Name", "ðŸ¢", "Your company name")
        self._phone = SI("Phone",        "ðŸ“ž", "+91 98765 43210")
        self._addr  = SI("Address",      "ðŸ“", "123 Business St, City")
        self._gst   = SI("GST Number",   "ðŸ§¾", "22AAAAA0000A1Z5")
        ...
        self._footer.setPlaceholderText("Invoice footer, terms, or notesâ€¦")
        ...
        btn = GB("Save & Continue  â†’", full=True); btn.clicked.connect(self._save)
        vl.addWidget(btn); vl.addStretch()
    ...
    def _remove(self):
        self._logo_b64 = ""
        self._logo_lbl.clear(); self._logo_lbl.setText("ðŸ¢")
        self._rm.setVisible(False)
...
# â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
#  VIEW 3  â€”  Signup
#  Mode A (is_first=True):  ADMIN creation â€” username locked, master code required
#  Mode B (is_first=False): regular user  â€” free username,  admin TOTP required
# â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
class V_Signup(LV):
    ...
    def __init__(self):
        ...
        self._un = SI("Username", "ðŸ‘¤", "Choose a username")
        self._un.setVisible(False)
        vl.addWidget(self._un); vl.addWidget(gap(10))

        self._pw  = SI("Password",         "ðŸ”’", "Strong password (12+ chars)", pw=True)
        self._cpw = SI("Confirm Password", "ðŸ”’", "Repeat password",             pw=True)
        ...
        # Auth code field â€” label changes per mode
        self._ms = SI("", "ðŸ›¡ï¸", "6-digit code", pw=True)
    ...
    def set_mode(self, is_first: bool, admin_totp_secret: str = ""):
        ...
        if is_first:
            ...
            self._notice_lbl.setText(
                "ðŸ”°  You are creating the ADMIN account.\n"
    ...
    def _strength(self, t: str):
        ...
        txt = "Strong âœ“" if n > 10 else ("Medium" if n > 6 else "Weak")
...
class V_QR(LV):
    ...
    def __init__(self):
        ...
        vl.addWidget(L("Recovery Codes â€” save all 8 before continuing",
        ...
        btn = GB("âœ“  I've saved my codes â€” Continue", "success", full=True)
...
class V_Login(LV):
    ...
    def __init__(self):
        ...
        self._pw = SI("Password", "ðŸ”’", "Enter your password", pw=True)
        ...
        nb = QPushButton("Create account â†’")
        nb.setStyleSheet(
            f"color:{C['accent']};background:transparent;border:none;"
            "font-size:12px;font-weight:600;")
    ...
    def refresh_users(self, users: list):
        self._combo.clear()
        self._combo.addItem("ðŸ‘¤  Select user")
        for u in users: self._combo.addItem(u)

    def _login(self):
        u  = self._combo.currentText()
        pw = self._pw.text()
        if u.startswith("ðŸ‘¤") or not pw:
...
class V_OTP(LV):
    ...
    def __init__(self):
        ...
        shield = QLabel("ðŸ›¡ï¸"); shield.setFont(F(26))
        ...
        vl.addWidget(L("â€” or enter a recovery code â€”", sz=10, col=C["text3"],
                       align=Qt.AlignmentFlag.AlignCenter))
        vl.addWidget(gap(8))
    ...
    def _adv(self, idx, t):
        if t:
            if idx < 5:
                ...
                # Last digit â€” auto-verify if all boxes filled
                code = self.otp_code()
                if len(code) == 6 and code.isdigit() and self._auto_verify_enabled:
                    self._otp_status.setText("â³  Verifyingâ€¦")
...
class _DashboardPage(QWidget):
    ...
    _CARDS = [
        ("ðŸ§¾", "New Sale",        "sale"),
        ("ðŸ“¦", "Products",        "product"),
        ("ðŸšš", "Suppliers",       "suppliers"),
        ("ðŸ›’", "Purchase Orders", "purchase_orders"),
        ("âš ï¸",  "Low Stock",      "low_stock"),
        ("ðŸ‘¥", "Customers",       "customers"),
        ("ðŸ’³", "Credit Mgmt",     "credit"),
        ("ðŸ’°", "P & L Summary",   "pl_summary"),
        ("ðŸ’¸", "Expenses",        "expenses"),
        ("ðŸ§¾", "GST Breakdown",   "gst"),
        ("ðŸ“Š", "Day Book",        "day_book"),
        ("ðŸ“¤", "Export",          "export"),
    ]
    ...
    def __init__(self, navigate_cb, company_name: str = "", username: str = "",
        ...
        greet_col.addWidget(L(f"Welcome back, {username} ðŸ‘‹", 11, col="#ffd6db"))
        greet_col.addWidget(L(name_disp, 20, col="#ffffff", bold=True))
        bl.addLayout(greet_col)
        bl.addStretch()

        badge = QLabel("âš¡ EvoAura")
        badge.setFont(F(13, bold=True))
        badge.setStyleSheet(
        ...
        # â”€â”€ Section header â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
_PAGE_META: dict[str, tuple[str, str]] = {
    ...
    "sale":             ("ðŸ›’", "New Sale"),
    "returns":          ("â†©ï¸", "Returns"),
    "bill_view":        ("ðŸ§¾", "Bill View"),
    "product":          ("ðŸ“¦", "Products"),
    "products":         ("ðŸ“¦", "Products"),
    "purchase_orders":  ("ðŸ“‹", "Purchase Orders"),
    "suppliers":        ("ðŸ­", "Suppliers"),
    "low_stock":        ("âš ï¸", "Low Stock"),
    "customers":        ("ðŸ‘¥", "Customers"),
    "credit":           ("ðŸ’³", "Credit Management"),
    "loyalty":          ("â­", "Loyalty"),
    "pl_summary":       ("ðŸ“Š", "P & L Summary"),
    "expenses":         ("ðŸ’¸", "Expense Tracking"),
    "gst":              ("ðŸ§¾", "GST Breakdown"),
    "cashflow":         ("ðŸ’°", "Cash Flow"),
    "day_book":         ("ðŸ“’", "Day Book"),
    "stock_report":     ("ðŸ“ˆ", "Stock Report"),
    "trial_balance":    ("âš–ï¸", "Trial Balance"),
    "export":           ("ðŸ“¤", "Export"),
    "company_settings": ("ðŸ¢", "Company Settings"),
    "users":            ("ðŸ‘¤", "Users & Roles"),
    "security":         ("ðŸ”", "Security"),
    "audit_log":        ("ðŸ—‚ï¸", "Audit Log"),
}
...
class AppShell(QWidget):
    ...
    def setup(self, username: str, db_name: str, users: int, sessions: int,
              company_name: str = ""):
        self._db_name = db_name          # â† stored for real page construction
    ...
    def _build_page(self, key: str) -> QWidget:
        ...
        # â”€â”€ Placeholder for all other keys â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        icon, title_text = _PAGE_META.get(key, ("ðŸ“„", key.replace("_", " ").title()))
...
class MainWindow(QMainWindow):
            ...
            try:
                ...
                if co:
                    if self._db.users_exist():
                        ...
                        self._toast("Database exists â€” no users yet. Go sign up.", "info")
    ...
    def _after_master(self):
        ...
        self.panel.show_features()
        self._sync_db_to_github()  # new DB just created â€” push it to GitHub
    ...
    def _verify_otp(self):
        ...
        if not verified:
            ...
            self._v_otp._otp_status.setText("âŒ  Invalid code â€” try again")
            self._toast("Invalid code. Try again.", "error"); return
        self._v_otp._otp_status.setText("âœ…  Verified!")
        self._session_count += 1
        self._toast(f"Welcome back, {self._pending_user}! ðŸŽ‰", "success")
