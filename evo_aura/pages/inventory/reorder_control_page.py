...
class LowStockPage(QWidget):
    def __init__(self, db_name, current_user="Admin", parent=None):
        ...
        title = QLabel("Low Stock Â· Reorder Control Center")
        title.setStyleSheet(
        ...
        self.kpi_labels = {}
        specs = [
            ("low", "Low Stock Items", "âš ï¸", C["warning"]),
            ("out", "Out of Stock", "â›”", C["accent"]),
            ("qty", "Reorder Qty Needed", "ðŸ“¦", C["blue"]),
            ("value", "Estimated Purchase Value", "ðŸ’°", "#AF52DE"),
            ("suppliers", "Suppliers Involved", "ðŸ­", "#00A67E"),
            ("critical", "Critical Items", "ðŸ”¥", "#FF3B30"),
        ]
        ...
        self.search.setPlaceholderText("Product name, code or barcodeâ€¦")
        ...
        self.add_stock_btn = _button("ðŸ“¦ Add Stock")
        self.create_invoice_btn = _button("ðŸ§¾ Create Purchase Invoice", True)
        self.view_product_btn = _button("ðŸ‘ View Product")
        self.view_supplier_btn = _button("ðŸ­ View Supplier")
        self.snooze_btn = _button("â° Snooze 7 Days")
    ...
    def refresh(self, *_):
        ...
        for source in self.rows:
            ...
            values = [
                ...
                source.get("priority"), "ðŸ‘  ðŸ“¦  ðŸ§¾  â°"]
    ...
    def _show_drawer(self, product):
        ...
        self.drawer_details.setText(
            f"<b>Code:</b> {product.get('item_code') or 'â€”'}<br>"
            f"<b>Category / Brand:</b> {product.get('category') or 'â€”'} / "
            f"{product.get('brand') or 'â€”'}<br>"
            f"<b>Size / Color:</b> {product.get('size') or 'â€”'} / "
            f"{product.get('color') or 'â€”'}<br>"
            f"<b>Supplier:</b> {product.get('list_supplier') or 'â€”'}<br><br>"
            ...
            f"<b>Last Invoice:</b> {product.get('last_invoice_number') or 'â€”'}<br>"
            f"<b>Last Purchase:</b> {product.get('last_purchase_date') or 'â€”'}<br>"
    ...
    def _view_product(self):
        ...
        QMessageBox.information(
            ...
            f"Category: {product.get('category') or 'â€”'}\n"
            f"Brand: {product.get('brand') or 'â€”'}\n"
    ...
    def _view_supplier(self):
        ...
        QMessageBox.information(
            ...
            f"Code: {code}\nPhone: {supplier.get('phone') or 'â€”'}\n"
            f"GSTIN: {supplier.get('gstin') or 'â€”'}\n"
